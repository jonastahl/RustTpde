...
fn main() {
  ...
  // 64-bit immediates that do not fit in a sign-extended imm32. These fail
  // today: RustCompilerX64.cpp emits `ASM(CMP64ri, lhs_reg, imm)` unguarded,
  // and x86-64 has no `cmp r64, imm64`, so the immediate is silently truncated
  // -- `cmp_imm_u64_wide` assembles to `cmp $0x0,%rdi`. Both need the immediate
  // materialised into a register and compared with CMP64rr instead.
